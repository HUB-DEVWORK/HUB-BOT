...
## Параллельная работа (несколько сессий/человек в одном дереве)

- Перед коммитом смотри `git status`: в дереве могут быть незакоммиченные правки
  ДРУГОЙ сессии/человека. **Коммить только свои файлы явным списком**, не `git add -A`.
- Общие файлы (container.py, tasks.py, enums.py) легко «утягивают» чужие строки в твой
  коммит — прод падал из-за импорта ещё не существующего модуля. Проверяй diff перед add.
- **Деплой — только из чистого worktree закоммиченной ревизии**
  (`git worktree add /tmp/deploy <rev>`), никогда из грязного дерева.
- Перед push всегда `git pull --rebase` (человек в команде тоже пушит).

## Целевой репозиторий

`github.com/bini69-oi/HUB-BOT`. Ветка по умолчанию — `main`. Коммитить по ходу работы
(разрешено явно), гейт перед коммитом — `make check` (или scoped-проверки при чужих правках в дереве).
