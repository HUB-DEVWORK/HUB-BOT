...
рассчитанным на будущую рантайм-установку без переписывания.
...
Типовая раскладка модуля: `modules/<Name>/` с `__init__.py`
...
`src/bot/hooks.py` — тонкая шина хуков:
...
- Результаты — список truthy-возвратов.
...
Дальше набор расширяем по мере переноса модулей (ориентир — ~2 десятка точек:
...
1. Первый «настоящий» модуль после message_cleaner — какой нужен раньше
   (notifications / раскладка меню / другое)?
2. Писать модули под HUB-BOT с нуля по образцу или переносить готовые
   (с проверкой лицензии источника)?
