...
export default function Modules() {
  ...
  async function restartWeb() {
    if (!(await confirm(t.modulesRestartWebAsk))) return;
    try {
      const r = await api.post<{ status?: string; hint?: string }>(
        "/api/admin/modules/restart-web",
      );
      if (r.status === "started") {
        toast(t.modulesRestartWebStarted);
      } else {
        toast(r.hint || r.status || "…");
      }
    } catch (e) {
